...
%
% See original Fortran code by Goffe for documentation and literature references.

%Open file to write temperature and step size
%fid = fopen('stepsize.out','w');
tic
LB=LB(:)';
UB=UB(:)';

s = RandStream('mt19937ar','seed',rseed);                               %sets seed for random number generator
sa_neps=4;                                %number of times eps
                                          %tolerance is achieved before termination
sa_eps=1e-2;                              %convergence criteria
sa_maxeval=1000000;                        %maximum number of function evaluations
  ...
  for m=1:sa_nt
    for j=1:sa_ns
      for h=1:sa_nargs
        ...
        xp=x;
        xp(h)=x(h)+VM(h)*(2*rand(s,1,1)-1.0);               %calculate new value for x (xp)
        if (xp(h)<LB(h)) | (xp(h)>UB(h))
          xp(h)=LB(h)+(UB(h)-LB(h))*rand(s,1,1);
          lnobds=lnobds+1;
          sa_nobds=sa_nobds+1;
        end
    ...
    %disp('No. of evaluations');disp(num2str(sa_nevals));
    disp(['Current temperature: ' num2str(sa_t)]);
  %  disp('current optimum function value');disp(num2str(fopt));
  %  disp('No. of downhill steps');disp(num2str(nup));  % note misnomer in variable declaration!
  %  disp('No. of accepted uphill steps');disp(num2str(ndown)); % we minimize, thus downhill is always accepted!
  %  disp('No. of rejections');disp(num2str(nrej));
...
%     save('lastxopt','xopt');
  ...
  if quit
    ...
    disp('optimum function value');disp(fopt);
    return
  end
