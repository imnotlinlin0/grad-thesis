...
%% Loads the news data
[a,b] = xlsread("D:\摸鱼\毕业论文\毕业论文\code\SVAR_data\news_all_with_sectors_and_filter_CN_2.xlsx");
a =[nan(1,size(a,2)+1);nan(size(a,1),1),a(:,:)];  %pad a to be same dimensions as b


%Drop all rows for dummy_merg = 0 and dummy_us = 0
pp_mrg  = logical((a(:,9) == 1).*(a(:,11)==1));
a       = a(pp_mrg,:);
b       = b(pp_mrg,:);



%% Function - make news weights (适配2007-2018年年度数据、12个行业)
...
pp_list = {'经济日报', '证券时报',   '人民日报',  '光明日报'};
pp_wghts = [.25,.25,.25,.25];  %Weights for each paper

pp_fil  = a(:,end) == 1; %filtered


weights_all  = cell(4,12);
weights_filt = cell(4,12);
for jj = 1:4 %For each paper
    
    pp_idx = strcmp(b(:,5), pp_list{jj});  %Which paper?
    tmp1 = make_news_wghts(a(pp_idx                 ,:));
    tmp2 = make_news_wghts(a(logical(pp_idx.*pp_fil),:));
    
    for ii = 1:12  %Store results for each sector
...
wghts_all = cell(1,12); wghts_all_filt = cell(1,12); wghts_jjrb_zqsb_filt = cell(1,12);
for ii = 1:12
...
for ii = 1:12
    wghts_jjrb_zqsb_filt{ii} = ts_waverage(pp_wghts(1:2),weights_filt{1:2,ii}) ;
end
